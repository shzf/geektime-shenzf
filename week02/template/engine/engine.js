...
export default class Engine {
  ...
  parseToNode(template) {
    ...
    while (stack.length > 0) {
      ...
      [...nodestr.matchAll(re)].forEach((item) => {
        ...
        let newn = new Vnode(n.tag, n.attr, [], pnode, n.childrenTemplate, null);
  ...
  //         let key = pnode.attr.get('v-if').trim()
  //         let isShow = false
  //         if (key.includes('.')) {
  //           isShow = scope
  ...
  parseNodeToDom(root, data) {
    ...
    while (stack.length > 0) {
      ...
      if (pnode.attr.get('for')) {
        this.processFor(pnode, pdom, scope, stack, data)
      } else if (pnode.attr.get('v-if')) {
        this.processVif(pnode, pdom, scope, stack, data)
      } else {
        this.processDefault(pnode, pdom, scope, stack, data)
      }
    }
    return fragment
  }

  processFor(pnode, pdom, scope, stack, data) {
    let [key, prop] = pnode.attr.get('for').split('in');
    key = key.trim();
    prop = prop.trim();
    for (let i = 0; i < scope[prop].length; i++) {
      ...
      let newScope = {};
      newScope[key] = scope[prop][i];
      this.processDefault(newnode, pdom, newScope, stack, data)
    }
  }

  processVif(pnode, pdom, scope, stack, data) {
    let key = pnode.attr.get('v-if').trim();
    let isShow = false;
    if (key.includes('.')) {
      isShow = scope;
  ...
  processDefault(pnode, pdom, scope, stack, data) {
    let html = this.scopehtmlParse(pnode, data, scope);
    let ele = this.createElement(pnode, html, scope, data);
    this.scopeAtrrParse(ele, pnode, data, scope);
    pdom.appendChild(ele);
  ...
  createElement(node, html,scope, data) {
